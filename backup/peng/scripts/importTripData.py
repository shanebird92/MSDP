...
def main(file):
    ...
    lineArray = []
    #  ['TripId', 'DataSource', 'DayOfService', 'LineId', 'RouteId',
    #   'Direction', 'PlannedTime_Arr', 'PlannedTime_Dep', 'ActualTime_Arr',
    #   'ActualTime_Dep', 'Basin', 'TenderLot', 'Suppressed',
    #   ]
    row = {}
    count = 0
    while(len(line) != 0):
        ...
        try:
            ...
            continue
        try:
            x.execute("insert into RT_Trips_2016(TripId, DataSource, DayOfService, LineId, RouteId, Direction, PlannedTime_Arr, PlannedTime_Dep, ActualTime_Arr, ActualTime_Dep, Basin, TenderLot, Suppressed) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11],row[12]))
            count += 1
        except Exception as e:
            print(str(e))
